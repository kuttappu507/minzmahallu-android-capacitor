/*
 * Runtime schema reconciliation — shared by the Electron desktop app and the
 * Capacitor Android web-DB layer (src/webapi/db.web.ts).
 *
 * Pure SQL against a better-sqlite3-shaped connection: NO Node or Electron
 * imports here, so the browser bundle can include it safely.
 */
export type SchemaDb = { prepare(sql: string): any; exec(sql: string): any };

import { installTokenDateGuard } from "./token-guard.js";

function columns(database: SchemaDb, table: string): Set<string> { return new Set((database.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>).map(x => x.name)); }
function addColumn(database: SchemaDb, table: string, name: string, definition: string) { if (!columns(database, table).has(name)) database.exec(`ALTER TABLE ${table} ADD COLUMN ${name} ${definition}`); }

/** Reconcile fields and feature tables used by the current CRUD layer before migrations run. */
export function ensureRuntimeSchema(database: SchemaDb) {
